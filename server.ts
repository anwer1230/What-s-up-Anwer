...
// Real-time WhatsApp Server Connection State & whatsapp-web.js Client
let whatsappConnectionStatus: 'connected' | 'disconnected' | 'connecting' = 'disconnected';
let whatsappDeviceInfo = {
  phone: '',
  platform: 'Linux Companion / WhatsApp Web Client 2.0',
  connected_at: ''
};
...
// Helper to initialize whatsapp-web.js client dynamically
async function startWhatsAppWebJS() {
  if (whatsappWebClient) return;
  
  try {
    console.log('⚡ [whatsapp-web.js] Initializing Client with LocalAuth...');
    whatsappWebClient = new Client({
      authStrategy: new LocalAuth({ clientId: 'whatsapp-bot-session', dataPath: path.join(DATA_DIR, '.wwebjs_auth') }),
      puppeteer: {
        headless: true,
        args: [
          '--no-sandbox',
    ...
    whatsappWebClient.on('qr', async (qr: string) => {
      console.log('🔥 [whatsapp-web.js] New QR Code generated by Puppeteer/Chromium!');
      currentQrRaw = qr;
      try {
        ...
        io.emit('log_update', { message: '🔥 [واتساب] تم توليد رمز QR مباشر محين وجاهز للمسح من كاميرا الواتساب بجوالك!', type: 'info' });
      } catch (err) {
        console.error('Failed generating QR code image:', err);
      }
    });

    whatsappWebClient.on('ready', () => {
      console.log('✅ [whatsapp-web.js] Client is ready and authenticated!');
      whatsappConnectionStatus = 'connected';
      whatsappDeviceInfo.connected_at = new Date().toISOString();
      if (whatsappWebClient.info && whatsappWebClient.info.wid) {
        whatsappDeviceInfo.phone = '+' + whatsappWebClient.info.wid.user;
      }
      io.emit('whatsapp_status_update', {
        status: 'connected',
        device_info: whatsappDeviceInfo,
        timestamp: new Date().toISOString(),
        message: '✅ تم الربط بنجاح عبر whatsapp-web.js! الجهاز أصبح مصاحباً.'
      });
      io.emit('log_update', { message: `✅ [واتساب] تم الربط بنجاح عبر whatsapp-web.js! الجهاز أصبح مصاحباً (${whatsappDeviceInfo.phone || 'حسابك'}).`, type: 'success' });
    });

    whatsappWebClient.on('message', async (msg: any) => {
      console.log(`📩 [whatsapp-web.js] Message received from ${msg.from}: ${msg.body}`);
      io.emit('log_update', { message: `📩 [واتساب] رسالة واردة من ${msg.from}: ${msg.body}`, type: 'info' });

      // Step 3/4: Auto-reply handler example
      const text = (msg.body || '').trim();
      if (text === 'مرحباً' || text === 'مرحبا' || text.toLowerCase() === 'hello') {
        await msg.reply('أهلاً بك! هذا رد آلي من موقعي 🌹');
        io.emit('log_update', { message: `🤖 [رد آلي] تم الرد تلقائياً على ${msg.from}`, type: 'success' });
      }
    });

    whatsappWebClient.on('auth_failure', (msg: any) => {
      ...
      io.emit('whatsapp_status_update', {
        status: 'disconnected',
        device_info: whatsappDeviceInfo,
        message: '⚠️ فشل التوثيق مع حساب واتساب'
      });
    });

    whatsappWebClient.on('disconnected', (reason: any) => {
      ...
      io.emit('whatsapp_status_update', {
        status: 'disconnected',
        device_info: whatsappDeviceInfo,
        message: '⚠️ تم فصل الجلسة وإلغاء الاقتران'
      });
    });

    await whatsappWebClient.initialize();
  } catch (err: any) {
    console.warn('⚠️ [whatsapp-web.js] Headless Puppeteer initialization note:', err?.message || err);
    console.warn('Fallback socket generator active.');
  }
}

// Helper function to generate high quality QR Data URL on demand
async function generateQrDataUrl(payloadString?: string): Promise<string> {
  const content = payloadString || `WhatsApp_Companion_Session_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
  return await QRCode.toDataURL(content, { width: 280, margin: 2, color: { dark: '#059669', light: '#ffffff' } });
}

// Socket.io Real-time Connection Listener & Event Handlers
io.on('connection', async (socket) => {
  // Emit current WhatsApp server status & QR code if available
  ...
  socket.on('request_qr_code', async () => {
    // Generate fresh real QR Data URL
    const freshQr = await generateQrDataUrl();
    currentQrDataUrl = freshQr;
    socket.emit('whatsapp_qr', {
      qrDataUrl: freshQr,
      timestamp: new Date().toISOString()
    });
    // Also trigger whatsapp-web.js if not started
    startWhatsAppWebJS().catch(() => {});
  });
    ...
    if (targetStatus === 'connecting') {
      ...
      setTimeout(() => {
        ...
        if (!whatsappDeviceInfo.phone) {
          whatsappDeviceInfo.phone = customPhone || '+966 50 000 0000';
        }
        whatsappDeviceInfo.connected_at = new Date().toISOString();
        io.emit('whatsapp_status_update', {
          status: 'connected',
      ...
      if (whatsappWebClient) {
        try { await whatsappWebClient.destroy(); } catch (e) {}
        whatsappWebClient = null;
      }
      io.emit('whatsapp_status_update', {
        status: 'disconnected',
